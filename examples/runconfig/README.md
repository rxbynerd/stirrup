...
| [`none-mcp-only.json`](none-mcp-only.json) | The `none` executor: no local filesystem or shell access at all — for an MCP-only / server-side-tool run. Read-only `planning` mode, one MCP server, and `tools.builtIn` limited to `web_fetch` (the only built-in tool that needs no executor capability). Not to be confused with `executor.network.mode: "none"` — see the callout in [`docs/configuration.md`](../../docs/configuration.md#components). |
