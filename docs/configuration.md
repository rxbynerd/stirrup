...
`stirrup config explain <field-path>` prints the documentation that
lives above each field in
[`types/runconfig.go`](../types/runconfig.go) — same source as this
document, surfaced through a generated lookup table so the CLI can
answer "what does this one field do?" without sending operators to
grep the source.

```sh
# Drill into a single field, including default and accepted values.
stirrup config explain mode
stirrup config explain provider.batch.maxWaitSeconds

# Expand a struct or wildcard prefix.
stirrup config explain provider.batch
stirrup config explain "provider.*"

# Discover everything: every leaf path, alphabetised.
stirrup config explain --list

# Machine-readable shape for scripting.
stirrup config explain mode --output=json
```

Unknown paths exit non-zero with a Levenshtein "did you mean ...?"
suggestion when one is close enough; the same `--list` output drives
shell completion for the field-path argument. The lookup is
regenerated by `just gen-docs` and policed by `just verify-docs` in
CI — the same drift contract `just proto` provides for the generated
gRPC sources.
