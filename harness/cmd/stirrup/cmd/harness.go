...
//
// debugRedactionDisabled and wireTrace carry --debug / --trace-wire and
// are doubly gated: validateDebugBuildFlags (PreRunE) rejects a release
// binary before either can be true, and core.BuildLoop re-checks the
// build tag at the point of effect. See docs/security.md#debug-builds.
...
func runWithConfig(config *types.RunConfig, opts runOptions) error {
	// Fires on every run of a debug binary, whether or not --debug or
	// --trace-wire were passed: the build tag alone is the signal
