...
// production callers outside the CLI pass none.
...
func BuildLoopWithTransport(ctx context.Context, config *types.RunConfig, tp transport.Transport, opts ...LoopOption) (*AgenticLoop, error) {
	...
	// The --trace-wire bit is re-gated against the build tag at the point
	// of effect so a release binary never installs a WireTapTransport
	// regardless of what a caller passes to WithWireTrace. See
