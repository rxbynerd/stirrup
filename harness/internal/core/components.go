...
//
// debugRedactionDisabled threads the --debug bit into the trace emitter.
// buildTraceEmitter re-checks it against the debug build tag, so passing
// true has no effect in a release binary; Preflight always passes false.
