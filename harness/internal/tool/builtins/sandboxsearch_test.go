...
// convention on the grep fallback: an exit code of 2 or higher is a real
// error (bad regex, IO failure), not "no matches", and must surface as a
// clear tool error rather than an empty result.
...
// when find itself errors inside the sandbox.
